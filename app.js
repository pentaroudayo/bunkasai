...
bgmPlayer.addEventListener('loadedmetadata', () => {
  ...
  const state = {
    ...
    config: {
      ...
      explosion: { name: '未選択', url: null },
      fastThresholdMs: 90 * 1000,
      normalThresholdMs: 3 * 60 * 1000,
      rewardVideos: {
        fast: { name: '未選択', url: null },
        normal: { name: '未選択', url: null },
        slow: { name: '未選択', url: null }
      }
    },
    buffer: '',
  ...
  dom.rewardOverlay.addEventListener('click', () => stopRewardVideo(true));
  ...
  function playErrorTone() {
    // Check if errorTone is ready to play
    if (errorTone.readyState >= 3) { // HAVE_FUTURE_DATA or HAVE_ENOUGH_DATA
      playClip(errorTone, { volume: state.config.sfxVolume / 100 });
    } else {
      // If not ready, try to load it and then play
      errorTone.load(); // Explicitly load
      errorTone.addEventListener('canplaythrough', function handler() {
        playClip(errorTone, { volume: state.config.sfxVolume / 100 });
        errorTone.removeEventListener('canplaythrough', handler); // Remove listener after playing
      });
    }
  }
